...
public class ColorThresholdPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        var materials = ColorThresholdMaterials.Instance;
        if (materials == null)
        {
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<ColorThresholdComponent>();
        // Only process if the effect is active
        if (customEffect.IsActive())
        {
            var material = materials.customEffect;
            // P.s. optimize by caching the property ID somewhere else
            material.SetColor(Shader.PropertyToID("_ColorThresold"), customEffect.colorThresold.value);
